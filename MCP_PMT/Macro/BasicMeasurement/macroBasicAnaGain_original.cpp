...
void macroBasicAnaGain_original(char fname[], bool automode=false)
{
...
//        if (((ihv-i)<=4 || gain[c][i]<2.5) && hv_max < hv[i]) hv_max = hv[i];
    ...
    f_gain[c]->SetParameters((TF1*)(gROOT->FindObject("expo"))->GetParameter(0),(TF1*)(gROOT->FindObject("expo"))->GetParameter(1));
  ...
  h_av->Fit("expo");
  double par_exp_av[2];
  for (int i=0; i<2; i++) par_exp_av[i] = (TF1*)(gROOT->FindObject("expo"))->GetParameter(i);
  c3->Update();
